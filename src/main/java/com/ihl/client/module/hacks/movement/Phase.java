        ...
        if (event instanceof EventPlayerMotion) {
            ...
            if (e.type == Event.Type.POST) {
                if (Helper.player().isCollidedHorizontally && Helper.player().isCollidedVertically && (sneak && Helper.player().isSneaking())) {
                    double val = (Helper.player().rotationYaw + 90 + (Helper.player().moveForward > 0 ? (Helper.player().moveStrafing > 0 ? -45 : Helper.player().moveStrafing < 0 ? 45 : 0) : Helper.player().moveForward < 0 ? 180 + (Helper.player().moveStrafing > 0 ? 45 : Helper.player().moveStrafing < 0 ? -45 : 0) : 0 + (Helper.player().moveStrafing > 0 ? -90 : Helper.player().moveStrafing < 0 ? 90 : 0))) * Math.PI / 180;
